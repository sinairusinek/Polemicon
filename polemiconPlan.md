...
- **Egeret-internal threading (pilot extension, 2026-05-14).** After the Q.C cross-source finding (line 79), we ran a scoped Egeret-internal threading pass to test whether structural threading on letters recovers real polemics without recipient resolution. Pipeline: slice the existing joint cluster space → 77 Egeret-bearing clusters (69 cross-source) → author-pair edges within clusters (2-yr window, date-confidence gated `egeret_dates.confidence ∈ {high, medium}`) → post-filters (thread span ≤ 15 yr, ≥ 0.5 edges/doc, 3–30 docs, ≤ 50 authors/cluster) → 17 candidate threads → Stage B `cli_sonnet` with letters-tuned prompt (`STAGE_B_PROMPT_LETTERS`). **Result:** 3/17 (17.6%) confirmed polemic, all `internal`, zero `external_defense`. Confirmed threads: Shadal vs. Haskalah/Zunz (explicit), Haskalah vs. rabbinate (meta-polemic), Hasidism–Haskalah tensions (implicit) — all historically well-attested debates, so the method isn't hallucinating. **Negative result preserved as motivation for filters:** first pass with a 5-yr window and no post-filters yielded 3 mega-threads of 431/176/93 docs spanning 34–80 years via transitive author-pair chaining (A↔B within 5 yr, B↔C within 5 yr, etc.); connected components absorbed entire clusters. **What the 14 topical-only verdicts reveal:** Maskil literary networks, Hebrew Revival institutions, Odessa circle, scholarly publishing, Revisionist Zionist logistics — all are correspondence between collaborators, not adversaries. Author-pair edges alone cannot distinguish "co-authored project" from "opposing positions" without recipient identity. **Zero `external_defense` is expected:** Egeret is intra-Jewish correspondence; antisemitism-defense polemics would live in cross-corpus Egeret↔press edges, which are still blocked (line 79). **Close-read sanity check (2026-05-14, `logs/egeret_threading_close_read.md`):** Manual reading of the 3 polemic threads' core letters confirms all verdicts hold textually. Shadal explicitly breaks with Zunz over denial of Torah from Heaven (egeret_4466 quotes Zunz back at him); Gordon names Yehiel Brill of Ha-Levanon as the adversary in his self-described "war" against the rabbinate (egeret_3442, 3447); Gordon's open letter to the Ha-Magid editor cites issue 29 by number and accuses the publication of giving space to "empty words with not a kernel of truth" (egeret_3465). False-negative scan of the 14 topical-only verdicts: **zero false negatives at the thread level.** Stage B correctly distinguishes "thread contains a polemical letter" (which is common — Ben-Yehuda's rebuttal of Ha-Po'el Ha-Tza'ir in thread 100011, Gordon's quarrel with Ha-Magid in thread 100010, Luzzatto's Sefer HaYashar attack in thread 100007) from "thread IS a polemical exchange" (where multiple authors engage each other). **Methodological reframe:** the 17.6% is *exchange-yield*, not *polemic-content yield*; doc-level polemic content lives in `full_corpus_predictions.parquet` and is the complementary view. The `outlier_docs` field on Stage B verdicts (commit 46c1be0) is doing useful work: even within confirmed-polemic threads, 25–30% of letters are flagged as off-topic clustering artifacts (sonnets, personal-news letters by the same author). Future improvement: pre-filter thread membership by Stage A `is_polemical` before computing connected components, which would tighten the confirmed-polemic threads from 16/19/3 letters down to their 4/5/2-letter polemic cores. **Revisit by:** (a) building the `egeret_N` ↔ E-GERET NER letter-id (`27904-0`) mapping; (b) Hebrew name-variant entity matching against E-GERET's 9,814 NER persons (the `Sender`/`Recipient` fields in `e-geret-ner-export.tsv` are clean but the same person spans many variants — Mendelssohn appears as `משה מנדלסון` / `מאזעס מענדלסזאהן` / `משה מדעסא` / `משה מדעס ב"ה`); (c) only then can recipient→author edges and Egeret↔press mention edges be built. Files: `data/egeret_polemic_clusters.parquet`, `data/egeret_polemic_cluster_labels.parquet`, `data/egeret_threads.parquet`, `data/egeret_edges_authorpair.parquet`; pipeline in `scripts/build_egeret_clusters.py`, `scripts/build_egeret_threads.py`; full writeup in `logs/egeret_threading_summary.md`; app surface in `src/pages/Egeret_Clusters.py` (Threads tab).
